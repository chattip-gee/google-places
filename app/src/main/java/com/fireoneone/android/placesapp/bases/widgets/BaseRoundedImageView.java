...
public class BaseRoundedImageView extends BaseImageView {
    ...
    private ColorStateList borderColor =
            ColorStateList.valueOf(BaseRoundedDrawable.DEFAULT_BORDER_COLOR);
    ...
    public BaseRoundedImageView(Context context) {
        super(context);
    }

    public BaseRoundedImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BaseRoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (borderColor == null) {
            borderColor = ColorStateList.valueOf(BaseRoundedDrawable.DEFAULT_BORDER_COLOR);
        }
    ...
    public void setImageDrawable(Drawable drawable) {
        mResource = 0;
        mDrawable = BaseRoundedDrawable.fromDrawable(drawable);
        updateDrawableAttrs();
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = BaseRoundedDrawable.fromBitmap(bm);
        updateDrawableAttrs();
        super.setImageDrawable(mDrawable);
    }
    ...
    private Drawable resolveResource() {
        ...
        return BaseRoundedDrawable.fromDrawable(d);
    }
    ...
    private void updateBackgroundDrawableAttrs(boolean convert) {
        if (mutateBackground) {
            if (convert) {
                mBackgroundDrawable = BaseRoundedDrawable.fromDrawable(mBackgroundDrawable);
            }
    ...
    private void updateAttrs(Drawable drawable) {
        ...
        if (drawable instanceof BaseRoundedDrawable) {
            ((BaseRoundedDrawable) drawable)
    ...
    public void setBorderColor(ColorStateList colors) {
        ...
        borderColor =
                (colors != null) ? colors : ColorStateList.valueOf(BaseRoundedDrawable.DEFAULT_BORDER_COLOR);
        updateDrawableAttrs();
        updateBackgroundDrawableAttrs(false);
        if (borderWidth > 0) {
            invalidate();
    ...
    public void downloadCircleImage(final Context context, String url) {
        downloadImage(context, url, new DownloadImageCallback() {
            @Override
            public DrawableRequestBuilder<String> onBuild(DrawableRequestBuilder<String> drb) {
                return drb.bitmapTransform(
